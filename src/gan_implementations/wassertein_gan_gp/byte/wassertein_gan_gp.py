...
class WasserteinGANGradientPenalty(ByteHistogramGAN):
        ...
        try:
            while True:
                ...
                g_features, g_ember_features_v1, g_ember_features_v2, g_raw_paths, g_raw_npz_paths, g_y = next(
                    generator_iterator)
                d_data = next(discriminator_iterator)
                # print('成功生成迭代器')
                ############################
                ...
                ###########################
                train_step=training_parameters['train_every_X_steps']
                log_interval=training_parameters["log_interval"]
                eval_interval=training_parameters["eval_interval"]
                # print(f'i=={i}')
                # print(f'train_step: {train_step},log_interval: {log_interval},eval_interval: {eval_interval}')
                ...
                if i % training_parameters["eval_interval"] == 0:
                    ...
                    # 新添加的调试
                    a1=evasion_rates['fid']
                    b1=info['fid']['best']
                    print(f'evaluation_info[fid]:{a1},info[fid][best]:{b1}')
    ...
    def evaluate(self, epoch:int, i:int, criterions:dict, valid_labels:Variable, fake_labels:Variable, training_parameters:dict):
        ...
        evaluation_info["distance_metrics"]["jensenshannon"]["between_fake"].append(
            pdist(fake_features_arr.reshape(-1,1), metric="jensenshannon"))


        # print(np.concatenate(evaluation_info["intermediate_features"]["fake"]).shape)
        # print( np.concatenate(evaluation_info["intermediate_features"]["benign"]).shape)
    ...
    def train(self, training_parameters:dict, patience:int=5):
        ...
        for epoch in range(training_parameters["num_epochs"]):
            ...
            train_D_losses.append(sum(info["losses"]["train"]["D"]) / (len(info["losses"]["train"]["D"])+1e-6))
            train_G_losses.append(sum(info["losses"]["train"]["D"]) / (len(info["losses"]["train"]["D"])+1e-6))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--malware_histogram_features_filepath",
                        type=str,
                        help="Filepath to malicious import features",
                        default="../../../../npz/BODMAS/histogram_features/malicious/")# 注意以下修改过，多一个../
    ...
    parser.add_argument("--malconvgct_model_filepath",
                        type=str,
                        help="MalConv GCT model filepath",
                        default="../../../ml_models/torch_models/malconvgct_detector/models/malconvgct_model.h5")

    parser.add_argument('--malconvgct', dest='is_malconvgct_model', action='store_true')
    parser.add_argument('--no-malconvgct', dest='is_malconvgct_model', action='store_false')
    parser.set_defaults(is_malconvgct_model=False)
    ...
    parser.add_argument("--inception_checkpoint",
                        ...
                        default="../../../ml_models/torch_models/inception_detector/models/best/best_model.pt")
    # ../../../ml_models/torch_models/inception_detector/models/byte_histogram_model_2017/best/best_model.pt
    ...
    # ../../../ml_models/torch_models/inception_detector/intermediate_features/byte_histogram_2017.npz
    ...
    wassertein_gan.build_blackbox_detectors(
        ...
        args.nonneg_malconv,
        args.malconvgct_model_filepath,  # 新增参数
        args.is_malconvgct_model  # 新增参数
    )
